...
(function () {
            ...
            mount.innerHTML = rows.length
                ? `<table class="table"><thead><tr><th>Employee</th><th>Opened</th><th>Closed</th><th>Cash</th><th>Over/short</th><th></th></tr></thead><tbody>${rows
                      .map(
                        ...
                        <td><button type="button" class="btn btn-sm btn-secondary" data-shift-report="${s.id}">Report</button></td>
                    </tr>`
                      )
                      .join('')}</tbody></table>`
                : '<p style="color:var(--gray-500);">No shift sessions.</p>';
            mount.querySelectorAll('[data-shift-report]').forEach((btn) => {
                btn.addEventListener('click', () => printShiftReport(btn.getAttribute('data-shift-report')));
            });
        } catch (err) {
            mount.innerHTML = `<p style="color:var(--error);">${esc(err.message)}</p>`;
        }
    ...
    async function printShiftReport(id) {
        try {
            ...
            if (!w) return;
            w.document.write(`<!DOCTYPE html><html><head><title>Shift ${id}</title></head><body style="font-family:system-ui;padding:24px">
                <h1>Shift report #${id}</h1>
