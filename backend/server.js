...
app.use(helmet({
    contentSecurityPolicy: {
        directives: {
            ...
            scriptSrcAttr: process.env.NODE_ENV === 'production' ? ["'none'"] : ["'unsafe-hashes'"], // Allow event handlers in development
            fontSrc: ["'self'", "https://fonts.gstatic.com", "https://cdnjs.cloudflare.com"],
            imgSrc: ["'self'", "data:", "https:", "blob:"],
            connectSrc: ["'self'", "https://fonts.googleapis.com", "https://cdnjs.cloudflare.com", "https://hmherbs.com", "https://fonts.gstatic.com"],
            frameSrc: ["'none'"],
            objectSrc: ["'none'"],
            mediaSrc: ["'self'"],
            workerSrc: ["'self'"],
...
app.use(cors({
    origin: function (origin, callback) {
        // Allow requests with no origin or null origin (file:// protocol, mobile apps, curl requests)
        ...
        if (allowedOrigins.indexOf(origin) !== -1) {
            ...
            // In development, allow localhost origins even if not in the list
...
const limiter = rateLimit({
    ...
    skip: (req) => {
        ...
        if (process.env.NODE_ENV !== 'production') {
            // Skip for HTML, CSS, JS, images, fonts, etc.
            const staticExtensions = ['.html', '.css', '.js', '.jpg', '.jpeg', '.png', '.gif', '.svg', '.woff', '.woff2', '.ttf', '.eot', '.ico'];
...
app.get('/api/products', async (req, res) => {
    try {
        ...
        // Count query uses same params as main query (LIMIT/OFFSET are not in queryParams anymore)
...
// Public analytics endpoint (for frontend error tracking)
app.post('/api/analytics', (req, res) => {
    // Silently accept analytics data (can be logged or stored later)
