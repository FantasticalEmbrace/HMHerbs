...
class PWAManager {
    ...
    async registerServiceWorker() {
        ...
        if ('serviceWorker' in navigator) {
            try {
                ...
                // Only log in development mode (check if we're on localhost)
                const isDevelopment = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
                if (isDevelopment && this.config && this.config.debugMode) {
                ...
                if (!error.message.includes('URL protocol') && !error.message.includes('not supported')) {
    ...
    async requestNotificationPermission() {
        ...
        try {
            ...
            if (error.name === 'NotAllowedError' || error.name === 'TypeError') {
                // User denied permission or API not available - silently ignore
                localStorage.setItem('notification-permission-asked', 'true');
                return 'denied';
            }
            throw error;
    ...
    initializeAppInstall() {
        this.addEventListenerWithCleanup(window, 'beforeinstallprompt', (event) => {
            // Don't call preventDefault() - let browser show native install prompt
            // This prevents the "Banner not shown" warning
            // Store event in case we want to use it later
    ...
    showInstallPrompt() {
        ...
        this.addEventListenerWithCleanup(dismissBtn, 'click', () => {
            // Call prompt() when dismissing to satisfy browser requirement
            // This prevents the "Banner not shown" warning
            if (this.installPromptEvent) {
                // Prompt and immediately resolve to dismiss
                this.installPromptEvent.prompt().then(() => {
                    return this.installPromptEvent.userChoice;
                }).then((choiceResult) => {
                    // User dismissed, clear the event
                    this.installPromptEvent = null;
                }).catch(() => {
                    // If prompt fails, just clear the event
                    this.installPromptEvent = null;
                });
            }
            this.hideInstallPrompt();
        ...
        // Auto-hide after 10 seconds - call prompt() to satisfy browser
        setTimeout(() => {
            if (document.getElementById('app-install-banner')) {
                // Call prompt() when auto-hiding to prevent browser warning
                if (this.installPromptEvent) {
                    this.installPromptEvent.prompt().then(() => {
                        return this.installPromptEvent.userChoice;
                    }).then((choiceResult) => {
