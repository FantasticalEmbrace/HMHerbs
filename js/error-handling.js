...
class ErrorHandler {
    ...
    handlePromiseRejection(rejectionInfo) {
        // Ignore permission denied errors (expected for notifications, geolocation, etc.)
        if (rejectionInfo.reason && (
            rejectionInfo.reason.name === 'NotAllowedError' ||
            rejectionInfo.reason.name === 'PermissionDeniedError' ||
            (typeof rejectionInfo.reason === 'string' && rejectionInfo.reason.includes('Permission denied'))
        )) {
            // Silently ignore - these are expected when users deny permissions
            return;
        }

        // Ignore CORS errors for external images (expected)
        if (rejectionInfo.reason && (
            (typeof rejectionInfo.reason === 'string' && rejectionInfo.reason.includes('CORS')) ||
            (rejectionInfo.reason.message && rejectionInfo.reason.message.includes('CORS'))
        )) {
            return;
        }
    ...
    handleResourceError(resourceInfo) {
        // Ignore CORS errors for external images (expected)
        if (resourceInfo.source && resourceInfo.source.includes('hmherbs.com')) {
            // Silently ignore - these are expected CORS errors
            return;
        }
        
        // Only log non-CORS resource errors
