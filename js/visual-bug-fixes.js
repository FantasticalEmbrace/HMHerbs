...
class VisualBugFixer {
    ...
    handleNewImage(img) {
        ...
        const handleError = () => {
            ...
            // Only log errors for same-origin images - suppress external image errors
            try {
                const imgUrl = new URL(originalSrc, window.location.href);
                const isExternal = imgUrl.origin !== window.location.origin;
                if (!isExternal) {
                    console.warn('❌ Image failed to load, applying fallback:', originalSrc);
                }
                // Silently ignore external image errors - they're expected (CORS, network issues, etc.)
            } catch (e) {
        ...
        if (img.complete) {
            ...
            setTimeout(() => {
                if (this.loadingImages.has(img)) {
                    // Only log timeout for same-origin images
                    try {
                        const imgUrl = new URL(originalSrc, window.location.href);
                        const isExternal = imgUrl.origin !== window.location.origin;
                        if (!isExternal) {
                        ...
                        // Silently ignore external image timeouts
                    } catch (e) {
                        console.log('⏰ Image loading timeout:', originalSrc);
                    }
                    handleError();
