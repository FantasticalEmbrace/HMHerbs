...
class EDSABookingSystem {
    ...
    getApiBaseUrl() {
        ...
        if (window.location.protocol === 'file:') {
            ...
            // Return API URL for when server is running (backend on port 3001)
            return 'http://localhost:3001/api/edsa';
        }

        // Backend serves both frontend and API on port 3001
                            ...
                            <div class="edsa-form-section">
                                <form id="edsa-booking-form">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea id="edsa-notes" name="notes" rows="3" autocomplete="off"></textarea>
                                    </div>
    ...
    async loadBusinessHours() {
        ...
        const url = `${this.apiBaseUrl}/hours`;

        try {
            const nativeFetch = window.__nativeFetch || window.fetch;
            const response = await nativeFetch(url).catch(() => null);
            ...
            // Silently fail - use default hours
        }
    }
    ...
    async handleFormSubmit(e) {
        ...
        if (!window.HMHERBS_PHONE_US || !HMHERBS_PHONE_US.isValidDisplay(phoneRaw, false)) {
            alert('Please enter a valid phone in the format (601) 398-5600.');
            return;
        }
